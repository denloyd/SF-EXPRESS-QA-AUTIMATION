...
String filePath = 'C:\\Users\\denlo\\OneDrive\\Desktop\\OJT Tasks\\OJT related tasks\\OJT MANUAL TESTING\\SF CHI\\SF-EXP-SUWP-00005.xlsx'
...
WebUI.click(findTestObject('Page_e-Konek Apps - SF Status Uploader/button_Proceed'))
	...
	// success message 
WebUI.waitForElementVisible(findTestObject('Page_e-Konek Apps - SF Status Uploader/div_Saved successfully'), 10)

WebUI.verifyElementText(findTestObject('Page_e-Konek Apps - SF Status Uploader/div_Saved successfully'), 'Saved successfully.')
WebUI.comment(" Rows saved: " + excelRows.size())
excelRows.eachWithIndex { row, i ->
	WebUI.comment(" Saved Row ${i + 1} — HAWB: ${row.hawb} | EVENT: ${row.eventCode} | REASON: ${row.reasonCode} | DATE: ${row.date}")
}

WebUI.click(findTestObject('Page_e-Konek Apps - SF Status Uploader/button_OK'))
WebUI.comment(" TEST PASSED — All ${excelRows.size()} row(s) saved successfully to database")